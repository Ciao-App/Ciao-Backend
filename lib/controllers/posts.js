...
module.exports = Router()
  ...
  .post('/:id/favorite', [authenticate], async (req, resp, next) => {
    try {
  ...
  .delete('/:id/favorite', [authenticate], async (req, resp, next) => {
    try {
      const post = await Favorites.delete(req.user.id, req.params.id);
      if (!post) next();
      resp.status(200);
      resp.send();
    } catch (e) {
